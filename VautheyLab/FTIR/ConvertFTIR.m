...
% Ask the user what to do
    choice = questdlg('Convert all files from OPUS to CSV?', ...
        'Convert files?', ...
        'Yes, individual files','Yes, 1 file','No','No');
    switch choice
        case 'No'
            convert = 0;
        case 'Yes, individual files'
            convert = 1;
        case 'Yes, 1 file'
            convert = 2;
    end
    ...
    I = contains(allfiles,["dpt","csv","txt","dat",".m","DS_Store"]);
    files = allfiles(I==0);
    cd(ConvertDir);
    switch convert
    case 1
        % Create subdirectory "csv" and put files there
        mkdir csv; cd csv;
        % Read all files and write corresponding CSV
        for i=1:length(files)
            ...
            newname = [currfile,'.csv'];
            csvwrite(newname,[x,y])
        end
        cd(ConvertDir)
    ...
    case 2
        ...
        if calc == 1
                ...
                WL = inputdlg('Define the spectral window to save:','Define spectral window',1,{'1800 2200'});
